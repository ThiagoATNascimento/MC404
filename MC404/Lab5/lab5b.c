...
unsigned int slice(unsigned int input, int start_bit, int end_bit){
    input <<= (31 - end_bit);
    input >>= (31 - end_bit + start_bit);
    return input;
}


unsigned int shift(unsigned int input, int start_bit){
    input <<= start_bit;
    return input;   
}
...
int main()
{
    ...
    {
    case R:
        resposta += shift(data->funct7, 25) + 
        shift(data->rs2, 20) + 
        shift(data->rs1, 15) + 
        shift(data->funct3, 12) + 
        shift(data->rd, 7) + 
        shift(data->opcode, 0);
        break;
    
    case I:
        resposta += shift(slice(data->imm, 0, 11), 20) +
        shift(data->rs1, 15) +
        shift(data->funct3, 12) + 
        shift(data->rd, 7) + 
        shift(data->opcode, 0);
    ...
    case S:
        resposta += shift(slice(data->imm, 5, 11), 25) +
        shift(data->rs2, 20) + 
        shift(data->rs1, 15) + 
        shift(data->funct3, 12) + 
        shift(slice(data->imm, 0, 4), 7) +
        shift(data->opcode, 0);
        break;
    
    case B:
        resposta += shift(slice(data->imm, 12, 12), 31) +
        shift(slice(data->imm, 5, 10), 25) +
        shift(data->rs2, 20) + 
        shift(data->rs1, 15) + 
        shift(data->funct3, 12) + 
        shift(slice(data->imm, 1, 4), 8) +
        shift(slice(data->imm, 11, 11), 7) +
        shift(data->opcode, 0);
        break;
    
    case U:
        resposta += shift(slice(data->imm, 0, 20), 12) +
        shift(data->rd, 7) + 
        shift(data->opcode, 0);
        break;
    
    case J:
        resposta += shift(slice(data->imm, 20, 20), 31) +
        shift(slice(data->imm, 1, 10), 21) +
        shift(slice(data->imm, 11, 11), 20) +
        shift(slice(data->imm, 12, 19), 12) +
        shift(data->rd, 7) + 
        shift(data->opcode, 0);
        break;
    }
